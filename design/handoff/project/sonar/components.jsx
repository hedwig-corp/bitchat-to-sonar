...
function MsgList({ msgs, showAuthors, peerName, onClaim }) {
  ...
  return (
    <div className="bc-msgs" ref={ref}>
      ...
      {msgs.map((m, i) => {
        if (m.pay) return <PayBubble key={i} m={m} peerName={peerName} onClaim={onClaim ? () => onClaim(i) : null} />;
...
function Composer({ placeholder, transport, onSend, onPlus, onCommand }) {
  ...
  return (
    <div className="bc-composerwrap">
      ...
      <div className="bc-composer">
        ...
        <button
          className={'bc-sendbtn' + (text.trim() ? ' on' : '') + (transport === 'internet' ? ' net' : '')}
