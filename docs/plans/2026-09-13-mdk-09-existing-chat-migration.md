...
- `client::tests::incoming_09_named_pair_welcome_does_not_fold_three_member_room`
  (R-050: a recovered 3-person standup must not fold onto a 2-person
...
- `client::tests::incoming_09_dm_welcome_does_not_fold_recovered_room`
  (R-050 at the incoming-welcome call site: welcomer-only known
...
without touching `isDirect` (R-050). Room leave / mesh-folded delete
...
must still skip folding a recovered room onto a welcomer 1:1 (R-050).
...
npub (R-003 / R-050). Peer still on 0.8:
...
still must not fold onto a welcomer 1:1 (R-050). Pins:
...
fold onto a welcomer 1:1 (R-050). Pins:
`client::tests::incoming_09_dm_welcome_folds_recovered_08_direct_chat`.
...
historical transcripts — do not invent a fold (R-050). Hosts still
...
`refreshTranscriptGroupWindow`). Do not invent a fold (R-050). Pins:
`ConversationFoldTest.hasOlderForFoldFamilyReadsHiddenSibling`,
...
fold (R-050). Compose `quotedParentInFamilyCache` matches iOS
...
skipped `live_count < 3` (R-050 guard for DMs). Idle
...
unique subset (R-050). Restore also runs on `groups()`,
...
flipping them back to Pending (R-051). Do not include fold aliases of
