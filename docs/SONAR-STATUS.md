...
| `media` | Blossom upload bench: BUD-02 PUT + GET `descriptor.url` on `DEFAULT_BLOSSOM_SERVER` (`https://push.sonar.hedwig.sh`) vs candidate(s) (default `https://nostr.download`). Requires probe nsec (fail closed / Degraded without it — no HEAD-as-Ok). Probe nsec does **not** auto-enable `--chat-probe`. | **Done** | Probe nsec required (`SONAR_STATUS_PROBE_NSEC`); chat probe needs `--chat-probe` separately |
...
5. **Done:** sticker index probe + Blossom upload bench (primary Hedwig
   Blossom vs public `nostr.download`).
6. **Done:** switch app `DEFAULT_BLOSSOM_SERVER` to
   `https://push.sonar.hedwig.sh` after open-auth fix + upload latency win.
   Historical blobs on `nostr.download` (or any prior host) are **not** moved
   or deleted — messages store absolute `descriptor.url`, and clients fetch
   those URLs directly. The old host only matters for as long as it keeps the
   objects; new uploads go to Hedwig.
7. **Optional:** store daily uptime samples to drive real 90-day bars (replace
   `syntheticHistory`).
8. **Next (ops):** cut over DNS to `https://blossom.sonar.hedwig.sh` and flip
   the constant once that name is live.
