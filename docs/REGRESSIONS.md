...
**Partly guarded:** The journal's encrypted persistence, item-URL checkpoint, cleanup, and stable request identity are pinned independently. Apple media uploads use a lifecycle-leased media lane so slow network work cannot serialize relay-node installation behind it, but the host queue topology is review-guarded because `MarmotService` is not constructible in tests. No automated test kills and relaunches a native app in the middle of a real Blossom PUT; that remains device-level coverage.
