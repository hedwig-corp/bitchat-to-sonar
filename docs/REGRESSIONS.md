...
- **A background wake must close the Marmot store before iOS suspends the process (0xdead10cc, round 2).** #446 added `closeStoreAfterBackgroundWake()` but the close could not win the race, and 1.12.1 build 29 crashed with the same `RUNNINGBOARD 0xdead10cc` 31s after a background launch. Three things had to be true together, and each is a separate hazard worth keeping:
  ...
  - **The App Group flock must not be released only via the serial queue.** `closeNode()` releases `storeLock` inside its `workQueue` hop, i.e. behind whatever is parked there. `MarmotService.releaseStoreLockNow()` drops it synchronously under `nodeLock` first; `MarmotStoreLock.release()` is idempotent so the regular close may release again. Holding an `flock` on a shared-container file across suspension is the textbook 0xdead10cc trigger.
  ...
  The `withTimeout` around the close in `SonarPushProcessor` is a safety net, not the fix: it only guarantees the fetch completion handler fires, and it is `closeNode()`'s `withCheckedContinuation` that is not cancellation-aware, so a timed-out close keeps running.
