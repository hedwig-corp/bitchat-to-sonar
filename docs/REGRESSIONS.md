...
**The quiesce removed the only thing that healed a failed resume.** Cancelling
`syncTask` + `relayConnectTask` here is correct and stays, but it means the
scene-phase foreground resume is now the *sole* reconnect after this close — and
`connectLocal` refuses that resume behind the `nodeClosing` fence this close
raises, with no retry behind it. A quick background→foreground bounce therefore
left the app foregrounded with a nil node, painting local chat normally while
every relay call failed silently. The push-wake sibling has settled this since it
was written (`reconnectIfForegroundAfterStoreClose`); `suspendStoreForBackground`
now calls it too. Do not re-derive the quiesce as the culprit: the hazard is the
*unpaired* close, not the cancellation. See the "close must settle the foreground
resume" bullet under `## Unguarded`.
...
- **A store close must settle the foreground resume it can eat (iOS).** `closeNode()` raises the `nodeClosing` fence *before* it hops onto `workQueue`, and `connectLocal` refuses a connect behind that fence with `ServiceError.cancelled`. `performConnect` schedules no retry on that failure, and `ensureConnected(timeoutSeconds:)` kicked `connectIfNeeded()` exactly **once**, before its poll loop — so a resume that raced the close burned its only attempt against the fence and then polled a value that could never change. R-020 is what made this terminal rather than merely slow: quiescing `syncTask` + `relayConnectTask` on the scene-suspend path removed the accidental `scheduleRelayConnect` safety net, so after the close **no timer remains to heal it**. Recovery then depends on some *other* action that kicks `connectIfNeeded` — every send path's `ensureConnected(timeoutSeconds: 2)`, or the legacy chat list's `onAppear` — and the pay/descriptor chain had **no such hook at all**, so a session where the user taps Pay without sending stays broken indefinitely. It does not read as an outage, because local-first paint is working as designed — the transcript and chat list render from SQLCipher — while every relay-bound call throws `notConnected` into a swallowed `catch`. The visible symptom was the pay sheet: `fetchSonarDescriptorSync` → `performDescriptorFetch` discarded the throw, the caller could not tell it from "this peer published no offer", and `paymentDetailsUnavailableMessage` returned "Fetching payment details — try again in a moment." forever with nothing retrying underneath. **This is the second time the same race was fixed.** The push-wake path has carried `reconnectIfForegroundAfterStoreClose()` (né `…AfterWakeClose`) since it was written, with a comment naming the fence exactly; the scenePhase sibling `suspendStoreForBackground()` never got it — the same asymmetry, in the same pair of functions, that R-020 records for `syncTask` cancellation. Three changes: the scene close now runs the recheck too, `ensureConnected` re-kicks inside its poll loop (needed independently — the close can hop behind a 20s relay FFI and outlive the 10s window; the re-kick is fire-time gated on `canPrefetchFromRelays`, so a kick landing after a background transition cannot reopen the store the suspend hook already closed), and `performDescriptorFetch` logs the throw. The symptom fix for the pay sheet specifically is the bounded node+relay ensure in `fetchSonarDescriptorSync` — a device repro must tap Pay *without* sending first, because any send heals the node through its own `ensureConnected` and proves nothing about this path. **Unpinned:** nothing constructs `MarmotChatModel` with a controllable `MarmotService`, so neither the fence race nor the re-kick is reachable from a test, and iOS tests do not run in CI regardless (see below). The reconnect predicate is deliberately *not* extracted into `RelayConnectionPolicy` as a pure `foreground && !connected` helper — that would be an R-001-shaped overclaim, green while the wiring that actually broke (whether the scene close calls it at all) stayed unguarded. Closing this properly needs an injectable service seam.
