...
- **Apple BLE media completion is not recipient completion.** `TransferProgressManager.completed` currently means every fragment work item was scheduled; it does not prove CoreBluetooth accepted each write or that the recipient reassembled the file. Text already uses the router-owned outbox and delivery acknowledgements, so this gap is media-only. Follow up by carrying the transfer id through CoreBluetooth write/notify completion and a file-level recipient acknowledgement, then retry original bytes through the existing Marmot media path. Until then a Sonar-to-Sonar media send made on Apple can still appear sent when the peer disappears mid-transfer.
