...
**Invariant:** A kind-30447 recovery beacon (published after nsec-only restore) may re-invite its author into a FRESH group ONLY when (a) the survivor already shares a group with that author, (b) the beacon has not been processed before for that author (`created_at <= processed`), and (c) the beacon is not older than the newest decrypted inbound message from that author (`created_at < last_inbound_from`). Processing it retires the old 1:1 group, creates one new group, and emits exactly one `ConversationReset`; DM reuse then folds into the healed group. The durable processed watermark advances only when at least one heal succeeds (or nothing is actionable); transient KP/relay failures leave the beacon retriable. On the restored side, outstanding-beacon auto-accept stays open across multiple chats until the accept cap (`MAX_RECOVERY_AUTO_ACCEPTS`) or 48h TTL — it must not clear merely because any new group appeared. Beacons are published ONLY from the explicit restore path when Blossom account-backup restore did not succeed — never after a successful Blossom restore, never from ordinary reconnect, and never from fresh onboarding.
...
**Call sites:** core `client.rs::handle_recovery_beacon` / `fetch_and_process_recovery_beacons` / `heal_dm_via_recovery` / `publish_recovery_beacon_background` / `find_dm_group_with` (retired skip); `recovery.rs::RecoveryState::is_beacon_replayed_or_stale`. Restore publishers: iOS `MarmotChatView.restoreIdentity` → `publishRecoveryBeaconBackground`; Compose `SonarCore.importIdentity` arms `recovery.announcePending` → connect-path publish. Apps surface the notice: iOS `MarmotService`/`SonarAppStore`; Compose `SonarCore`/`SonarAppState`.
...
**Rejected:**
- *Publish the beacon on every KeyPackage republish (every connect).* Peers with a live chat would reset on each of the restorer's reconnects.
- *Publish the beacon whenever the local MLS store is empty (KP background path).* Fresh onboarding also has an empty store and would arm outstanding-beacon auto-accept for any multi-member group invite — gated to the explicit restore path instead.
