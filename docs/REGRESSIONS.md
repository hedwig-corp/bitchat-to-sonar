...
- **Ordinary `MeshRadio.stop()` must not discard undrained delivery signals.** With the Rust engine no longer queuing plaintext (`pending_sends` removed in favour of fail-fast `send_text`), the app router is the only retry owner. Clearing the Android failure/receipt inboxes inside `stop()` therefore lost the retry signal on every activity teardown (`MainActivity.onDestroy`), not just on erase/wipe, leaving the row "Sent" forever. The clear now lives in `MeshRadio.discardPendingDeliverySignals()`, called only from `wipe()`, `eraseAllChats()` and the account-restore teardown in `SonarAppState`. Unpinned on both platforms: `stop()` needs a real BLE stack, and the call-site wiring is exactly what regressed — a fourth teardown path that clears the inboxes directly would reintroduce it with every test still green.
...
- **`PendingMarmotOutbox` is tested but not wired.** The rebase onto main kept main's `mutableMapOf<String, MutableList<PendingMeshMarmotSend>>` for the folded-mesh Marmot fallback, so the typed FIFO in `SonarOutbox.kt` has no callers while `PendingMarmotOutboxTest` still exercises it. The ordered "failed head stays ahead of sends appended while it was in flight" guarantee that type exists for is therefore NOT in effect: a failed fallback send marks its echo "Couldn't send" and is dropped. Either wire the FIFO back in or delete it with its test — a green test over dead code is exactly the overclaim this ledger forbids.
- **Android asynchronous BLE send failures.** `MeshGatt` now carries app-owned text/media metadata beside every queued GATT write/notification, resets the Noise route on a rejected, failed, disconnected, or stuck operation, and exposes one failure for router fallback. Intentional shutdown advances a delivery generation and clears already-buffered app failures so erased plaintext/media cannot be resurrected as a fallback send. The Rust test `send_text_without_live_route_fails_instead_of_hiding_plaintext` pins the synchronous half, but no JVM test can drive `BluetoothGattCallback`; close this with an Android driver seam plus a two-device Pixel test that drops the link mid-fragment.
...
- **BLE receipt timeout/retry and desktop media.** Upgraded iOS/macOS and Compose Android receivers carry the sender's optional media message id, persist the completed file/transcript, and return the existing Noise-encrypted `delivered` receipt. Senders therefore distinguish transport acceptance (`Sent`) from recipient persistence (`Delivered`). What is still missing is a bounded no-receipt timeout plus idempotent retry/dedup: if a peer disappears after GATT accepts a fragment, the row can remain `Sent` indefinitely. Retrying before receiver-side message-id dedup and duplicate re-ACK support would create duplicate bubbles, so those must land together. The Compose Desktop BLE implementation handles text receipts but still returns `false` for `sendMeshMedia`; native macOS shares the Apple media implementation. This is an explicit platform gap, not evidence that desktop media passed the same path.
