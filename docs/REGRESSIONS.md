...
- **Sender-side no-receipt timeout is the last piece of BLE delivery state.** Upgraded iOS/macOS and Compose Android receivers carry the sender's optional media message id, persist the completed file/transcript, and return the existing Noise-encrypted `delivered` receipt, so senders distinguish transport acceptance (`Sent`) from recipient persistence (`Delivered`) for both text and media. The receiver halves needed to make a retry safe are now in place: Compose dedups incoming text and media by the sender-chosen wire id (never matching one of our own rows) and **re-ACKs** a duplicate instead of dropping it, and Apple keeps a bounded sender-scoped `seenPrivateFileMessageIDs` set that re-ACKs a re-sent file rather than storing a second row — necessary because Apple's packet-level dedup keys on `senderID-timestamp-type`, which a retry does not collide with. What is still missing is the *sender* half: a bounded no-receipt timeout that re-enters the ordered outbox under the same message id and gives up as "Couldn't send", gated on peers that have proven they ack (a stock bitchat peer never returns a media receipt, so an ungated timeout would either spam it or wrongly fail a delivered message). Until that lands a row whose receipt is lost stays `Sent` indefinitely — recoverable only by the user re-sending. None of the receiver dedup/re-ACK paths is pinned: all four sites need a constructed `SonarAppState` or a `BLEService` with a live peer. The Compose Desktop BLE implementation handles text receipts but still returns `false` for `sendMeshMedia`; native macOS shares the Apple media implementation. This is an explicit platform gap, not evidence that desktop media passed the same path.
