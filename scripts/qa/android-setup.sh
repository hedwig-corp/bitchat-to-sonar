...
AVD="Sonar_QA_API_36"; PORT=5580; INSTALL=1; FRESH=0; ALLOW_MISSING=0
while [[ $# -gt 0 ]]; do
  case "$1" in
    --avd) AVD="$2"; shift 2 ;;
    --port) PORT="$2"; shift 2 ;;
...
if (( ${#missing[@]} )); then
  ...
  for gap in "${missing[@]}"; do            # idempotent across setup reruns
    grep -qxF "android: $gap" "$QA_HOME/config-gaps.txt" 2>/dev/null ||
      echo "android: $gap" >> "$QA_HOME/config-gaps.txt"
  done
fi
