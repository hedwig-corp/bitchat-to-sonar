...
2. Faithful "existing account, cold process" run: `scripts/bench/provision-and-bench.sh --app "$APP" --runs 5 --msgs-per-run 3`. It seeds a real Marmot group via `sonar-cli` and pushes fresh messages before each run so every cold start exercises the real relay re-sync path (`woke=1`). For just the identity-independent phase breakdown, use `scripts/bench/cold-start-bench.sh --app "$APP" --runs 5`.
