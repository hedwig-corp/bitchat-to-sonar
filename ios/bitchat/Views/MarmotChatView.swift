...
enum SNMarmotChatSnapshotCache {
    ...
    static func load(from defaults: UserDefaults) -> ([MarmotService.MarmotGroup], [String: [MarmotService.MarmotMessage]]) {
        ...
        // a pre-isDirect blob before FFI groups() returns (R-050).
...
final class MarmotChatModel: ObservableObject {
    ...
    func loadOlderLocalPage(groupId: String) async -> Bool {
        ...
        // cursor for the hidden sibling (R-050).
        ...
        for sibling in unpaged {
            ...
            // extract snaps (R-050). Only newest-page an empty unpaged sibling.
    ...
    func title(for group: MarmotService.MarmotGroup) -> String {
        ...
        // Same short npub form as the pending chat and the lookup card, so the
        ...
        return snMarmotChatDisplayTitle(
            ...
            npubFallback: other.map { snShortNpubLabel($0) } ?? ""
        )
    }
