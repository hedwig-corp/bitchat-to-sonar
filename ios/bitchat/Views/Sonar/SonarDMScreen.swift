...
enum SNPeerLocalTimeFormatter {
    static func display(
        ...
        let amount = relativeAmount(minutes: abs(offsetSeconds) / 60, locale: locale)
    ...
    /// Matches design `bcZoneDelta`: whole hours as words, mixed as `5h 30m`.
    static func relativeAmount(minutes: Int, locale: Locale = .autoupdatingCurrent) -> String {
        ...
        if hours == 1 {
            return String(localized: "1 hour", locale: locale)
        }
        return String.localizedStringWithFormat(
            String(localized: "%@ hours", locale: locale),
            "\(hours)"
        )
    }
...
func snShareLocalTimeNote(sharing: Bool, overridden: Bool, zone: String, peerName: String) -> String {
    if sharing {
        return "Sharing \(zone) with \(peerName) inside this chat’s encryption. Only the timezone travels, never a location."
    }
    if overridden {
        return "Off for this chat — overrides your Settings default. Only the timezone travels, never a location."
    }
    return "Off — follows your Settings default. Only the timezone travels, never a location."
}
    ...
    var body: some View {
        ...
        .snSheet(isPresented: $paySheet, title: "Send money · \(peer.name)") {
            SNPaySheet(
                ...
                onSendMax: { sats in
                    Task {
                        if let message = await store.sendPay(peerId, sats: sats, feeFromAmount: true) {
                            showToast(message)
                        }
                    }
                }
