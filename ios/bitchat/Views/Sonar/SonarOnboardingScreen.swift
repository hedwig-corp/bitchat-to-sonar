    ...
    private var stepRestore: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            (Text("Paste your ") + Text("nsec").fontWeight(.bold)
                + Text(" private key. This restores your Sonar identity and wallet on this phone. Chat history is recovered from your encrypted Blossom backup when one exists for this key."))
                .font(SonarTheme.uiFont(size: 16))
                .lineSpacing(16 * 0.3)
                .foregroundColor(SonarTheme.text2)
