...
struct BLEServiceCoreTests {
    ...
    // A peer whose clock is wrong must still be discoverable. Every packet from
    // a device 153 days behind was rejected by the ±120s skew gate and the 900s
    // announce-age gate, so it was permanently invisible to Apple peers while
    // still seeing them — Android's engine never rejected on time, so the
    // failure was silent and one-way. Verified on a Pixel 4 XL whose clock had
    // drifted 5 months. Full-TTL identity packets came off a live link and are
    // fresh by construction; relayed copies keep both checks.
    @Test
    func announceFromPeerWithBadlySkewedClock_isStillDiscovered() async throws {
        let ble = makeService()

        let signer = NoiseEncryptionService(keychain: MockKeychain())
        let announcement = AnnouncementPacket(
            nickname: "Wrong Clock",
            noisePublicKey: signer.getStaticPublicKeyData(),
            signingPublicKey: signer.getSigningPublicKeyData(),
            directNeighbors: nil
        )
        ...
        let skewedMs = UInt64(Date().addingTimeInterval(-153 * 24 * 60 * 60).timeIntervalSince1970 * 1000)
        let announcePacket = try #require(signer.signPacket(BitchatPacket(
            type: MessageType.announce.rawValue,
            senderID: Data(hexString: peerID.id) ?? Data(),
            recipientID: nil,
        ...
        let didAppear = await TestHelpers.waitUntil({
            ble.currentPeerSnapshots().contains { $0.peerID == peerID }
        }, timeout: TestConstants.shortTimeout)
        #expect(didAppear)
    }

    // The exemption is scoped to the direct copy: a relayed announce (ttl < max)
    // is the path a stored/gossip replay actually travels, so it must still be
    // rejected on age or old announces resurrect ghost peers.
    @Test
    func relayedAnnounceWithSkewedClock_isStillRejected() async throws {
        let ble = makeService()

        let signer = NoiseEncryptionService(keychain: MockKeychain())
        let announcement = AnnouncementPacket(
            nickname: "Relayed Ghost",
            noisePublicKey: signer.getStaticPublicKeyData(),
            signingPublicKey: signer.getSigningPublicKeyData(),
            directNeighbors: nil
        )
        ...
        let staleMs = UInt64(Date().addingTimeInterval(-153 * 24 * 60 * 60).timeIntervalSince1970 * 1000)
        let relayed = try #require(signer.signPacket(BitchatPacket(
            type: MessageType.announce.rawValue,
            senderID: Data(hexString: peerID.id) ?? Data(),
            recipientID: nil,
            timestamp: staleMs,
            payload: announcePayload,
            signature: nil,
            ttl: 5 // decremented by a relay hop
        )), "Failed to sign announce packet")

        ble._test_handlePacket(relayed, fromPeerID: peerID, preseedPeer: false)
