...
import chat.bitchat.sonar.durablyRetireAndroidDirectory
...
actual object WalletBridge {
    ...
    /** Serializes irreversible SDK operations with shutdown/storage retirement.
     * The state [lock] is held only for short snapshots, never native calls. */
    ...
    actual suspend fun setupIfNeeded(nsec: String): Unit = withContext(Dispatchers.IO) {
        setupGate.withLock {
            ...
            // The native connect is not cooperatively cancellable. Bound only
            // this caller's wait; the detached attempt remains tracked so wipe
            // fails closed until it finishes and its stale node is disconnected.
        ...
        balanceListenerId = runCatching {
            node.addEventListener(object : EventListener {
                override fun onEvent(e: SdkEvent) {
                    // Never block the native callback thread. Account-bound side
                    // effects run under [lock], serialized with shutdown/panic.
                    walletScope.launch {
        ...
        withContext(Dispatchers.IO) {
            operationGate.withLock {
                ...
                try {
                    ...
                    // Last reversible point: a panic/setup generation change wins
                    // before Breez receives the irreversible send request.
    ...
    actual suspend fun wipeLocalData(): Boolean = cleanupGate.withLock {
        ...
        val retired = withContext(Dispatchers.IO) {
            operationGate.withLock {
                ...
                durablyRetireAndroidDirectory(
                    File(ctx.filesDir, "sonar-wallet"),
                    ".sonar-wallet-wipe-",
                )
            }
        }
    ...
    private fun cleanupPending(): Boolean =
        cleanupPrefs().getBoolean(CLEANUP_PENDING_KEY, false)
