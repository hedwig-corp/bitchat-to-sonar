...
/**
 * Desktop (JVM) `actual`: a 1:1 forward onto the generated `SonarCashuWallet`
 * (core/sonar-ffi/src/wallet.rs). No policy lives here — see
 * [CashuWalletEngine]. Kept in step with `CashuNative.android.kt`, which forwards
...
actual fun openCashuNative(nsec: String, mintUrl: String, workingDir: String): CashuNative =
    ffi {
        ...
        DesktopCashuNative(SonarCashuWallet(nsec, mintUrl, workingDir))
    }

actual fun fetchFiatRatesNative(): List<ExchangeRate> =
    ffi {
        SonarNativeLoader.ensureLoaded()
        fetchFiatRates().map { ExchangeRate(it.currency.uppercase(), it.perBtc) }
    }

actual fun walletStorageRoot(): String = DesktopEnv.dataDir.absolutePath

private class DesktopCashuNative(private val w: SonarCashuWallet) : CashuNative {
