 ...
 *  hist→stale still wins (R-050). Wake-mute stays FFI-wins.
 ...
 *  room does not open as "Group chat". Default not-direct (R-050). */
 ...
 *  Persist hist→stale still wins (R-050). Empty without a remount pair
 ...
 *  the remounted live extract (R-050). iOS `snHiddenFoldFamilyNeedsPage`. */
 ...
 *  onto live before hist had a newest page (R-050 — do not invent a fold).
 *  Empty persist-folds still union the remount pair. */
 ...
 *  sibling. Do **not** copy `isDirect` — R-050. */
...
/** Marmot `openChat` early paint: [firstOpenHasLocalTranscriptPaint], but
 *  never while something arrived since the chat was left (R-049,
 *  [reopenTranscriptPaint]) — the unread divider would anchor on the
 *  stale leave frame. That open loads the local page first instead. */
internal fun firstOpenPaintsBeforePage(
    retained: List<SonarMsg>,
    snapshot: List<SonarMsg>,
    unreadAtOpen: Long,
): Boolean = unreadAtOpen <= 0L && firstOpenHasLocalTranscriptPaint(retained, snapshot)
 ...
 *  [hist] wins (R-050). A first-resume blob for another chat must not
...
class SonarAppState(private val scope: CoroutineScope) {
    ...
    fun chatTitle(chat: SonarChat): String {
        ...
        // second 1:1 with the welcomer (R-050).
    ...
    fun openChat(chat: SonarChat, jumpMessageId: String? = null) {
        ...
        // not empty. Unless something arrived since leaving (R-049,
        // reopenTranscriptPaint): an unread open loads the local page
        // below — which still seeds the fold family — so the unread
        // divider anchors on fresh rows, not on the leave frame.
        val (opened, pane) = remountPairForOpenChat(chat.id)
        ...
        val openRetained = retainedTranscriptForChat(
            chat.id,
            retainedTranscriptByChat,
            historicalFoldMap,
            openedConversationId = opened,
    ...
    private fun clearSendEcho(chatId: String, echoId: String) {
        ...
        val (opened, pane) = remountPairForOpenChat(chatId)
        removePendingMessagesForChat(
            chatId,
            pendingSendEchoes,
            historicalFoldMap,
            shouldRemove = { it.id == echoId },
            openedConversationId = opened,
            openedConversationPaneId = pane,
        )
        val openId = (screen as? Screen.Chat)?.id
    ...
    private fun withSendEchoes(chatId: String, published: List<SonarMsg>): List<SonarMsg> {
        val (opened, pane) = remountPairForOpenChat(chatId)
        val familyEchoes = pendingMessagesForChat(
            chatId,
            pendingSendEchoes,
            historicalFoldMap,
            opened,
        ...
        if (plan.terminalAcceptedEchoIds.isNotEmpty()) {
            removePendingMessagesForChat(
                ...
                pendingSendEchoes,
                historicalFoldMap,
                shouldRemove = { it.id in plan.terminalAcceptedEchoIds },
                openedConversationId = opened,
                openedConversationPaneId = pane,
            )
