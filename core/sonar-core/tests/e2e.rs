...
/// `create_group` also has an empty desc — R-050). After a lost JSON
...
/// that as a DM, so hosts folded it onto the welcomer 1:1 (R-050).
#[tokio::test]
...
/// A timezone share is a control event (R-017): it must cache the peer zone through
...
async fn timezone_share_does_not_notify_or_increment_unread() {
    ...
    assert!(
        notes.is_empty(),
        "a timezone share must not produce a push notification"
    );
    assert_eq!(
        bob.messages(&bob_group).expect("bob messages").len(),
        1,
        "a timezone share is not a transcript row"
    );
    assert_eq!(
        bob.conversation_summary(&bob_hex)
            ...
            .unwrap_or(0),
        unread_before,
        "a timezone share must not increment unread"
    );
}
