//! Headless proof for the CDK backend. Conventions match `sonar-wallet-cli`:
//! newline-JSON on stdout, tracing on stderr, secrets from the environment
//! only (argv is visible in `ps` and lands in shell history), `derive` prints
//! a fingerprint unless explicitly asked for the full seed.
